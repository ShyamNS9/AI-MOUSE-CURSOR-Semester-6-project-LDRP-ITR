...
class HandDetector():
    ...
    def find_hands(self, img, imgx, to_draw=True, hand_name='Right'):  # Left or Right but first letter should be capital
        self.hand_n = hand_name  # to give hand name to other methods
        img = cv2.flip(img, 1)
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        self.results = self.abc.process(img_rgb)
        # This is all same as what we have learned in: ITER-2 AI Mouse Project V1 (trackers)/OCVhandtracker4.py
